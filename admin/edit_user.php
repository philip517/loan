...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_user'])) {
        ...
        $stmt = $pdo->prepare("UPDATE user_table SET username=?, email=?, first_name=?, last_name=?, phone=?, address=?, NRC=?, gender=?, occupation=?, date_of_birth=?, nationality=?, role=? WHERE user_id=?");
        $stmt->execute([$username, $email, $first_name, $last_name, $phone, $address, $NRC, $gender, $occupation, $date_of_birth, $nationality, $role, $target_user_id]);
    ...
    if (isset($_POST['toggle_status'])) {
        $new_status = $_POST['user_status'] === 'active' ? 'deactivated' : 'active';
        
        // Since we don't have a status column, we'll add one or use a different approach
        // For now, let's add a status column to user_table
        $stmt = $pdo->prepare("UPDATE user_table SET status = ? WHERE user_id = ?");
    ...
    if (isset($_POST['delete_user'])) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("DELETE FROM login_details WHERE user_id = ?");
...
// Check if status column exists, if not we'll handle it differently
$check_status_sql = "SHOW COLUMNS FROM user_table LIKE 'status'";
$status_column_exists = $pdo->query($check_status_sql)->rowCount() > 0;

// Get current status
if ($status_column_exists) {
    $status_sql = "SELECT status FROM user_table WHERE user_id = ?";
    $status_stmt = $pdo->prepare($status_sql);
    $status_stmt->execute([$target_user_id]);
    $user_status = $status_stmt->fetchColumn() ?: 'active';
} else {
    $user_status = 'active'; // Default if no status column
}
?>
<!DOCTYPE html>
